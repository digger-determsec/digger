...
//! protocol-model facts. Provenance is shared via `digger-gen5-common`.

/// Deterministic provenance for a DERIVED protocol-model fact.
/// Re-exported from `digger-gen5-common`.
pub use ::digger_gen5_common::derive_provenance;

/// Implements the reconstruction `RecoveredFact` trait for local model facts.
macro_rules! impl_protocol_fact {
    ($($t:ty),+ $(,)?) => {
